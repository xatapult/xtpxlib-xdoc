...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:variable name="dref-in" as="xs:string" select="xtlc:href-protocol-remove(xtlc:href-canonical(/*/xmldoc:document[1]/@dref))"/>
  <xsl:variable name="filename" as="xs:string" select="xtlc:href-name($dref-in)"/>
  ...
  <xsl:template name="generate-document-documentation">
    ...
    <xsl:for-each select="$document">
      ...
      <p>File: <code><xsl:value-of select="xtlc:href-protocol-remove(xtlc:href-canonical(@dref))"/></code></p>
      <br/>
