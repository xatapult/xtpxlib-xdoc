...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:map="http://www.w3.org/2005/xpath-functions/map"
  ...
  <xsl:template match="db:table[@xml:id] | db:informaltable[@xml:id]">
    ...
    <xsl:variable name="altcolspecs" as="element(xdoc:altcolspecs)*" select="key($altcolspecs-keyname, $id)[@target = $alttarget-names]"/>

    <xsl:choose>
      <xsl:when test="exists($altcolspecs)">
        <xsl:variable name="altcolspec" as="element(xdoc:altcolspecs)*" select="local:get-first-for-alttargets($altcolspecs)"/>
        <xsl:copy>
          <xsl:apply-templates select="@*"/>
          <xsl:comment> == colspecs set for target "{$altcolspec/@target}" == </xsl:comment>
          <xsl:apply-templates mode="mode-change-colspec">
            <xsl:with-param name="alt-colspecs" as="element(xdoc:colspec)*" select="$altcolspec/xdoc:colspec" tunnel="true"/>
          </xsl:apply-templates>
        </xsl:copy>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="db:colspec" mode="mode-change-colspec">
    ...
    <xsl:variable name="colspec-index" as="xs:integer" select="count(preceding-sibling::db:colspec) + 1"/>
    <xsl:variable name="alt-colspec" as="element(xdoc:colspec)?" select="$alt-colspecs[$colspec-index]"/>
    <xsl:choose>
  ...
  <xsl:template match="db:figure[@xml:id]" mode="#all">
    <xsl:variable name="id" as="xs:string" select="string(@xml:id)"/>
    <xsl:variable name="altimagedatas" as="element(xdoc:altimagedata)*" select="key($altimagedata-keyname, $id)[@target = $alttarget-names]"/>

    <xsl:choose>
      <xsl:when test="exists($altimagedatas)">
        <xsl:variable name="altimagedata" as="element(xdoc:altimagedata)?" select="local:get-first-for-alttargets($altimagedatas)"/>
        <xsl:copy>
          <xsl:apply-templates select="@*"/>
          <xsl:comment> == altimagedata set for target "{$altimagedata/@target}" == </xsl:comment>
          <xsl:apply-templates mode="mode-change-imagedata">
            <xsl:with-param name="alt-imagedata" as="element(xdoc:altimagedata)?" select="$altimagedata" tunnel="true"/>
          </xsl:apply-templates>
        </xsl:copy>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:function name="local:get-first-for-alttargets" as="element()?">
    ...
    <xsl:iterate select="$alttarget-names">
      ...
      <xsl:variable name="applicable-element" as="element()?" select="$elements[exists(@target)][@target eq $alttarget-name][1]"/>
