...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="xdoc:transform/@href[not(xtlc:href-is-absolute(.))]">
    ...
    <xsl:variable name="path-normalized" as="xs:string"
      select="normalize-space(.) => translate('\', '/')"/>
    <xsl:variable name="path-is-xdoc" as="xs:boolean"
      select="starts-with($path-normalized, $xdoc-indicator)"/>
    ...
    <xsl:variable name="root-path" as="xs:string">
      <xsl:choose>
        <xsl:when test="$path-is-xdoc">
          <xsl:sequence select="resolve-uri('../../transforms/', static-base-uri())"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:if test="not(doc-available($absolute-path-to-transform))">
      <xsl:call-template name="xtlc:raise-error">
        <xsl:with-param name="msg-parts"
          select="('xdoc transformation not found: ', xtlc:q($absolute-path-to-transform))"/>
      </xsl:call-template>
    </xsl:if>
    <xsl:variable name="transformation-extension" as="xs:string"
      select="xtlc:href-ext($absolute-path-to-transform)"/>
    <xsl:variable name="transformation-type" as="xs:string">
      <xsl:choose>
        <xsl:when test="$transformation-extension eq 'xpl'">
          <xsl:sequence select="'xproc1'"/>
        </xsl:when>
        ...
        <xsl:otherwise>
          <xsl:call-template name="xtlc:raise-error">
            <xsl:with-param name="msg-parts"
              select="'Could not determine transformation type for ', xtlc:q($absolute-path-to-transform)"
            />
          </xsl:call-template>
        </xsl:otherwise>
      </xsl:choose>
